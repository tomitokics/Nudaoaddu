...
#   tool by @dora_ios. Beta saving by @tomitokics
#
#   *test environment
...
if [ $# -lt 7 ]; then
    echo "./getshsh.sh <ios> <device model> <boardconfig> <ecid> <save-path> <BuildManifest.plist> <build id> [-q]"
    echo ""
    echo "[OPTION]"
    echo "-q        : quick mode (This flag is optional)"
    echo ""
    echo "example: ./getbetashsh.sh 12.1.1 iPhone6,2 N53AP 0x12345678 YourFolder BuildManifest.plist 16C5050a"
    exit
fi
    ...
    if [ "$6" = "-q" ]; then
        ...
        for nonce in ${ApNonce}
            do
                ...
                ./tsschecker -i --beta $1 --buildid $7 —d $2 --boardconfig $3 -e $4 -m $6 --save-path $5 -s --apnonce $nonce 2>/dev/null >/dev/null &
            done
        echo ""
    else
        echo "Invalid flag"
...
./tsschecker -i --beta $1 --buildid $7 —d $2 --boardconfig $3 -e $4 -m $6 --save-path $5 -s --apnonce 05fe405753166f125559e7c9ac558654f107c7e9
